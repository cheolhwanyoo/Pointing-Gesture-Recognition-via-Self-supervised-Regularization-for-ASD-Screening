...
def parse_args():
    ...
    parser.add_argument('--SSL', type=str, default='None', choices=['None', 'SimSiam', 'BYOL'])
...
if __name__ == '__main__':
    ...
    if args.SSL == 'None':
        checkpoint = torch.load('checkpoints/logs/' + args.model_name + '/model_best.checkpoint', map_location='cuda:0')
        model_PointDetNet.load_state_dict(checkpoint)

    else:
        state_dict = torch.load('checkpoints/logs/' + args.model_name + '/model_best.checkpoint', map_location='cuda:0')
    ...
    print('test_Accuracy: {:.3f}, recall: {:.3f}, precision: {:.3f}, f_score: {:.3f}'.format(acc, recall, precision, f_score))
    ...
    fp.write(
        ...
        ', TP: {}, FP: {}, TN: {}, FN: {}  \n'.
            format(args.model_name_arm, args.model_name, acc, recall, precision, f_score, TP, FP, TN, FN))
    fp.close()
