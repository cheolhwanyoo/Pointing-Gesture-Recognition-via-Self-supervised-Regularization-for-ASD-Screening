...
def parse_args():
    ...
    parser.add_argument('--batchsize', type=int, help='batch size', default=8)  # 50
    parser.add_argument('--epochs', type=int, help='training epochs', default=50)  # 50
    parser.add_argument('--SSL', type=str, default='None', choices=['None', 'SimSiam', 'BYOL'])  # 50
    parser.add_argument('--backbone', type=str, default='vit_B_32', choices=['resnet', 'vit_B_32'])  # 50
...
def train(model, criteria, optimizer, dataloaders, device=None, num_epochs=200):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch == 1 or epoch % args.test_freq == 0:
            ...
            test_acc = test_model(model, dataloaders, args, device, 'val')
            ...
            if test_acc[0] > test_acc_best:
                ...
                filename = (args.checkpointdir +
                            '/logs/' + args.model_name + '/model_best.checkpoint')
            ...
            if epoch == num_epochs:
                ...
                filename = (args.checkpointdir +
                            '/logs/' + args.model_name + '/model_%d.checkpoint' % epoch)
                torch.save(model.state_dict(), filename)
                print('Saving ' + filename)
...
def test_model(model, dataloaders, args, device, phase):
