...
def draw_humans(npimg, humans, human_idx):
    npimg = np.copy(npimg)
...
def write_humans_ych(npimg, humans, wr, imgcopy=False):
    if imgcopy:
        npimg = np.copy(npimg)
    image_h, image_w = npimg.shape[:2]
    centers = {}
    ...
    for human in humans:
        ...
        for i in range(CocoPart.Background.value):
            ...
            if i not in human.body_parts.keys():
                joint_list.append(0)
                joint_list.append(0)
                wr.writerow(joint_list)
                joint_list.clear()
                continue

            body_part = human.body_parts[i]
            joint_list.append(int(body_part.x * image_w + 0.5))
            joint_list.append(int(body_part.y * image_h + 0.5))
            wr.writerow(joint_list)
            joint_list.clear()
