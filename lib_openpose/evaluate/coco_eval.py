...
import torch
from lib_openpose.datasets.preprocessing import (inception_preprocess,
                                              rtpose_preprocess,
                                              ssd_preprocess, vgg_preprocess)
from lib_openpose.network import im_transform
from lib_openpose.config import cfg, update_config
from lib_openpose.utils.common import Human, BodyPart, CocoPart, CocoColors, CocoPairsRender, draw_humans
...
def get_outputs_openpose(img, model, preprocess):
    ...
    ########### ONNX  ###########
    # dummy_input = torch.randn(1, 3, 368, 656, requires_grad=False).cuda()
    #
    ...
    #                   input_names=['input1'], output_names=['output1', 'output2'], dynamic_axes={'input1': {2:'height', 3:'width'} })

    # # onnx export 모델과의 값 차이 확인
    # ort_session = onnxruntime.InferenceSession("openpose.onnx")
    #
    # def to_numpy(tensor):
    ...
    # print(ort_outs[0].astype(np.float32))
    # print(ort_outs[1].astype(np.float32))
    #
